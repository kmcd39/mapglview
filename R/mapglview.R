...
#'
#' @description A flexible function to internally get breaks according to a given style.
#' 
...
#' 
#' @export get.mapgl.interpolator
#' 
get.mapgl.interpolator <- function(
    x, zcol
  ...
  z <- dplyr::pull(x, zcol) 
  # make a list object so formatted legend breaks can be included.
  interpolator <- list()
  ...
  if( class(z) %in% 
    ...
    interpolator$legend.breaks <- breaks
    
    # continuous z
  } else if( class(z) %in% 
             c("numeric", "integer")) 
    {
    breaks <- get.mapglview.breaks(z, ...)
    pal <- grDevices::colorRampPalette(palette)(length(breaks))
...
#'
#' @description
#' Wrapper to quickly make a map using `mapgl`
#' 
#'
...
#'
#' @inheritDotParams get.mapglview.breaks
#' @inheritDotParams get.mapgl.interpolator
#'
#' @export mapglview
#' 
mapglview <- function(
  ...
  if( is.null(zcol) )
    ...
    interpolator <- get.mapgl.interpolator(x, zcol, ...)
  }
  ...
  if(grepl("POLYGON|COLLECTION|SURFACE|TIN|TRIANGLE", geom_type) ) {
    m <- m |> 
      mapgl::add_fill_layer(
        id = layer.name,
        source = x,
        fill_color = interpolator$interpolator,
        ,popup = "popup"
        ,tooltip = zcol
        ,before_id = before_id
        ,fill_opacity = alpha
  ...
  ### LINESTRINGS/CURVES --> LINE LAYER ####
    ...
    m <- m |> 
      mapgl::add_line_layer(
        id = layer.name,
        source = x,
        line_color = interpolator$interpolator,
  ...
  ### POINTS --> CIRCLE LAYER ####
    ...
    m <- m |> 
      mapgl::add_circle_layer(
        id = layer.name,
        source = x,
        circle_color = interpolator$interpolator,
  ...
  if(include.legend & !is.null(zcol)) {
    m <- m |> 
      mapgl::add_legend(
        ...
        ,values = interpolator$legend.breaks
        ,colors = interpolator$colors
        ,add = F #
        ,type =
